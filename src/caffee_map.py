...
def load_and_analyze_data():
    ...
    category_mapping = dict(
        zip(area_category_df['category'], area_category_df[' struct'].str.strip())
    )
    print('카테고리 매핑:')
    ...
    # area 1만 필터링 후 출력 (반달곰 커피가 있는 지역)
    ...
    # 전체 데이터 저장
    merged_df.to_csv('result/full_map.csv', index=False)

    print('파일 저장 완료: result/full_map.csv')
...
def main():
    try:
        ...
        print('생성된 파일:')
        print('- result/full_map.csv: 전체 지역 데이터')
